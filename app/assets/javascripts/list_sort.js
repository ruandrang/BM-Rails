  ...
  function initSortableList(listSelector, itemSelector, defaultDirections) {
    ...
    sortButtons.forEach(btn => {
      ...
      btn.addEventListener("click", function() {
        ...
        // data-member-id → dataset.memberId (DOMStringMap은 camelCase로 변환)
        const datasetKey = sortKey.replace(/-([a-z])/g, (_, c) => c.toUpperCase());
        ...
        sortButtons.forEach(b => {
          ...
          b.textContent = b.dataset.sortBaseLabel || b.textContent.trim().split(" ")[0];
        });

        this.classList.add("btn-active", "btn-primary", "text-primary-content");
        const arrow = sortState[sortKey] === "asc" ? " ↑" : " ↓";
        this.textContent = (this.dataset.sortBaseLabel || baseLabel) + arrow;
      });
    });
  }

  window.initSortableList = initSortableList;
})();
